...
export async function checkSensorState(
  ...
  if (
    ...
    alarmpromises.push(
      raiseEvent({
        type: "critical",
        message: `Alarm triggered by sensor at ${previousState.name} in ${
          previousState.building
        } with ip address of ${
          previousState.ipAddress
        }. \n This was detected at ${new Date().toString()}`,
        system: "backend:sensors:funcs",
        title: `Alarm Triggered: ${previousState.name} in ${previousState.building}`,
      })
    );
 ...
 * @throws {CustomError} If no config is found.
 *
...
export async function checkSensorTemperature(
  ...
  if (!config) {
    const err = raiseError(500, "No config found");
    throw err;
  }
