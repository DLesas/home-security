...
const PORT = process.env.UDP_BROADCAST_PORT
  ? Number(process.env.UDP_BROADCAST_PORT)
  : 41234;

/** The interval (in milliseconds) between broadcast messages */
const INTERVAL_MS = process.env.UDP_BROADCAST_INTERVAL
  ? Number(process.env.UDP_BROADCAST_INTERVAL)
  : 10000;
    ...
    interfaces?.forEach((iface) => {
      if (iface.family === "IPv4" && !iface.internal) {
        ...
        socket.on("error", async (error: Error) => {
          console.error(
            `UDP Broadcasting socket initialization error on ${iface.address}:`,
            error
          );
        ...
        const broadcast = () => {
          socket.send(message, PORT, BROADCAST_ADDR, async (error) => {
            if (error) {
              console.error(
                `UDP Broadcasting socket send error on ${iface.address}:`,
                error
              );
              await db.insert(errorLogsTable).values({
        ...
        socket.bind({ address: iface.address }, () => {
          ...
          console.log(
            `Broadcasting message ~${message}~ via udp on interface with address ${
              iface.address
            } on port ${PORT} every ${INTERVAL_MS / 1000}s`
          );
